...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  function hbeLoader() {
    ...
    // 检查旧的 Storage
    // 机制是输入密码后存到 Storage 中，然后显示加密之后的解密界面，此时web是只读没有任何缓存刷新
    // 手动刷新缓存后，才会访问到 post 对应的 js
    // 在 post 里插入 js <script> 手动删除 Storage，这样如果再刷新就必须重新输入密码
    // 但是我在下面的 if 里手动删了，一样
    if (oldStorageData) {
